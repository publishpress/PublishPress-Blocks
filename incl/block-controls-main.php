	...
	class Controls {
			...
			if ( Utilities::settingIsEnabled( 'block_controls' )
				...
				foreach ( $controls as $key => $item ) {
					if ( isset( $item['control'] )
					     && self::getControlValue( $item['control'],
							1 ) === true // Is this control enabled? @TODO Dynamic way to define default value depending the control ; not all will be active (1) by default
		...
		public static function checkBlockControlsWidget( $instance ) {
			...
			if ( strpos( wp_get_raw_referer(), '/wp-admin/widgets.php' )
			     ...
			     && false !== strpos( filter_var( wp_unslash( $_SERVER['REQUEST_URI'] ), FILTER_SANITIZE_URL ),
					'/wp-json/' )
			) {
				return $instance;
			}
				...
				case 'schedule':
					...
					if ( $dateFrom || $dateTo || $days || ( $timeFrom && $timeTo ) ) {
						...
						if ( $recurring ) {
							...
							$dateFrom->setDate( $nowFrom->format( 'Y' ),
								$dateFrom->format( 'm' ),
								$dateFrom->format( 'j' ) );
		...
		public static function editorAssets( $wp_editor_dep ) {
			if ( Utilities::settingIsEnabled( 'block_controls' ) ) {
				wp_enqueue_script(
					'advgb_block_controls',
					ADVANCED_GUTENBERG_PLUGIN_URL . '/assets/blocks/block-controls.js',
					[
						'wp-blocks',
						'wp-i18n',
						'wp-element',
