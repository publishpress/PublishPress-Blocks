    ...
    float: left;'
		...
		public function __construct() {
			...
			add_action( 'admin_print_scripts',
				array( $this, 'disableAllAdminNotices' ) ); // Disable all admin notice for page belong to plugin
		...
		public function replaceEditorSettings( $settings ) {
			...
			if ( method_exists( $current_screen, 'is_block_editor' ) && $current_screen->is_block_editor()
			     && ( ! defined( 'GUTENBERG_VERSION' ) || ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION,
							'5.3.0',
							'>=' ) ) ) ) {
		...
		public function addEditorAssets( $force_loading = false ) {
			$current_screen = get_current_screen();
			if ( ! $force_loading && method_exists( $current_screen,
		...
		public function addEditorAssetsSiteEditor() {
			...
			if ( $this->settingIsEnabled( 'enable_advgb_blocks' ) && $pagenow === 'site-editor.php' ) {
				...
				add_editor_style( plugins_url( 'assets/css/blocks.css',
					dirname( __FILE__ ) ) ); // 'advgb_blocks_styles'
				add_editor_style( plugins_url( 'assets/css/columns-editor.css',
					dirname( __FILE__ ) ) ); // 'advgb_blocks_editor_styles'
				add_editor_style( plugins_url( 'assets/css/recent-posts.css',
					dirname( __FILE__ ) ) ); // 'advgb_recent_posts_styles'
				add_editor_style( plugins_url( 'assets/css/editor.css',
					dirname( __FILE__ ) ) ); // 'advgb_editor_styles'
				add_editor_style( plugins_url( 'assets/css/site-editor.css',
					dirname( __FILE__ ) ) ); // Site editor iframe styles only
				add_editor_style( plugins_url( 'assets/css/fonts/material-icons.min.css',
					dirname( __FILE__ ) ) ); // 'material_icon_font'
				add_editor_style( plugins_url( 'assets/css/fonts/material-icons-custom.min.css',
					dirname( __FILE__ ) ) ); // 'material_icon_font_custom'
				add_editor_style( plugins_url( 'assets/css/slick.css', dirname( __FILE__ ) ) ); // 'slick_style'
				add_editor_style( plugins_url( 'assets/css/slick-theme.css',
					dirname( __FILE__ ) ) ); // 'slick_theme_style'
		...
		public function enqueueEditorAssets() {
			...
			if ( defined( 'ADVANCED_GUTENBERG_PRO' ) ) {
				...
				if ( method_exists( 'PPB_AdvancedGutenbergPro\Utils\Definitions',
					'advgb_pro_enqueue_scripts_editor_conditional' ) ) {
		...
		public function advgbBlocksVariables( $post = true ) {
			...
			$avatarHolder         = plugins_url( 'assets/blocks/testimonial/avatar-placeholder.png',
				ADVANCED_GUTENBERG_PLUGIN );
			$default_thumb        = plugins_url( 'assets/blocks/recent-posts/recent-post-default.png',
				ADVANCED_GUTENBERG_PLUGIN );
			...
			$rp_default_thumb     = isset( $saved_settings['rp_default_thumb'] ) ? $saved_settings['rp_default_thumb'] : array(
				'url' => $default_thumb,
				'id'  => 0
			);
			$icons                = array();
				...
				add_filter(
					...
					function ( $settings ) {
						$settings['advanced-gutenberg'] = [
							'message' => __( 'You\'re using PublishPress Blocks Free. The Pro version has more features and support. %sUpgrade to Pro%s',
								'advanced-gutenberg' ),
			...
			if ( $advgb_blocks_user_roles ) {
				foreach ( $advgb_blocks_user_roles as $role => $blocks ) {
					if ( is_array( $blocks ) && is_array( $blocks['active_blocks'] ) && is_array( $blocks['inactive_blocks'] ) ) {
						...
						if ( $newAllowedBlocks ) {
							$advgb_blocks_user_roles_updated[ $role ]['active_blocks']   = array_merge( $blocks['active_blocks'],
								$newAllowedBlocks );
			...
			if ( $task === '' ) {
				...
				foreach ( $data_saved as $data ) {
					if ( $data['id'] === $style_id ) {
						$copied_styles = array(
							'id'            => $new_id['id'] + 1,
							'title'         => sanitize_text_field( $data['title'] ) . ' ' . __( 'copy',
									'advanced-gutenberg' ),
				...
				if ( ! preg_match( $regex, $new_classname ) ) {
					wp_send_json( 'Please use valid characters for a CSS classname! As example: hyphen or underscore instead of empty spaces.',
						403 );
		...
		public function saveContactFormData() {
			...
			if ( $saved ) {
				...
				$msg            = '<html><body>';
				$msg            .= '<h2>' . __( 'You have received a contact from your website.',
						'advanced-gutenberg' ) . '</h2>';
				$msg            .= '<ul>';
				$msg            .= '<li><strong>' . __( 'Name',
						'advanced-gutenberg' ) . ': </strong>' . $contact_data['name'] . '</li>';
				$msg            .= '<li><strong>' . __( 'Email',
						'advanced-gutenberg' ) . ': </strong>' . $contact_data['email'] . '</li>';
				$msg            .= '<li><strong>' . __( 'Date',
						'advanced-gutenberg' ) . ': </strong>' . $contact_data['date'] . '</li>';
				$msg            .= '<li><strong>' . __( 'Message',
						'advanced-gutenberg' ) . ': </strong>' . $contact_data['msg'] . '</li>';
				$msg            .= '</ul>';
				$msg            .= '</body></html>';
		...
		public function handleLoginFailed() {
			...
			if ( ! empty( $_SERVER['HTTP_REFERER'] ) && $from_advgb ) {
				$redirect = add_query_arg( 'login',
					'failed',
					$_SERVER['HTTP_REFERER'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		public function featuresBoxes( $features ) {
			...
			echo( defined( 'ADVANCED_GUTENBERG_PRO' ) ? 'ispro' : 'isfree' ) ?>">
				...
				foreach ( $features as $feature ) : ?>
					...
					echo( ! $feature['access'] ? ' advgb-feature-box--disabled' : '' ); ?>">
                        <h3>
							<?php
							echo $feature['title'];
                        ...
                        <div class="advgb-feature-setting">
                            <div class="advgb-switch-button">
                                <label class="switch">
									...
									echo( ! $feature['access'] ? ' slider--disabled' : '' ); ?>"></span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <footer>
                        ...
                        <div class="advgb-pressshack-logo">
                            <a href="https://publishpress.com" target="_blank" rel="noopener noreferrer">
                                <img src="' . esc_url( plugins_url( 'assets/images/publishpress-logo.png',
						dirname( __FILE__ ) ) ) . '"
		...
		public function advgb_settings_save_page() {
            ...
            elseif ( isset( $_POST['save_email_config'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Missing -- we check nonce below
			{
				...
				if (
					! wp_verify_nonce(
						sanitize_key( $_POST['advgb_email_config_nonce_field'] ),
						'advgb_email_config_nonce'
					)
				) {
					return false;
				}
            ...
            elseif ( isset( $_POST['save_recaptcha_config'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Missing -- we check nonce below
			{
				...
				if (
					! wp_verify_nonce(
						sanitize_key( $_POST['advgb_captcha_nonce_field'] ),
						'advgb_captcha_nonce'
					)
				) {
					return false;
            ...
            elseif ( isset( $_POST['block_data_export'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Missing -- we check nonce below
			{
				if (
					! wp_verify_nonce(
						sanitize_key( $_POST['advgb_export_data_nonce_field'] ),
						'advgb_export_data_nonce'
					)
				) {
					return false;
				}
		...
		public function blocksFeatureSave( $feature, $option ) {
			...
			// Verify nonce
			if ( ! wp_verify_nonce( sanitize_key( $_POST[ 'advgb_block_' . $feature . '_nonce_field' ] ),
				'advgb_nonce' ) ) {
				return false;
			}
            ...
            <div class="publishpress-admin wrap">
				...
				if ( isset( $_GET['save'] ) && $_GET['save'] === 'success' ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- display message, no action ?>
                    <div id="message" class="updated fade">
                        <p>
							<?php
							printf( __( '%s saved successfully!', 'advanced-gutenberg' ), $label ); ?>
                        </p>
                    </div>
					<?php
                ...
                <div class="wrap">
                    <form method="post">
						<?php
						wp_nonce_field( 'advgb_nonce', 'advgb_block_' . $feature . '_nonce_field' ); ?>
                            ...
                            <div>
                                <select name="user_role" id="user_role">
									...
									foreach ( $roles_list as $roles => $role_name ) :
										...
										?>
                                        <option value="<?php
										echo esc_attr( $roles ); ?>" <?php
										selected( $current_user_role, $roles ); ?>>
											<?php
		...
		private function downloadBlockFormData() {
			// Verify nonce
			if ( ! wp_verify_nonce( sanitize_text_field( $_POST['advgb_export_data_nonce_field'] ),
				'advgb_export_data_nonce' ) ) {
				return false;
			}
		...
		public function loadBlockConfigView( $block = '' ) {
			...
			if ( ! isset( $blocks_settings_list[ $current_block ] ) ) {
				wp_die( esc_html( __( 'Default configuration for this block is not available.',
					'advanced-gutenberg' ) ) );
		...
		public function advgb_getNestedBlocksStyles( $block, $level = 2, &$style_html = array() ) {
			if ( isset( $block['innerBlocks'] ) ) {
				foreach ( $block['innerBlocks'] as $key => $inner_block ) {
					// Get styles
					$new_style_html = $this->advgb_SetStylesForBlocks( $inner_block['attrs'],
		...
		public function setFrontendAssets( $content ) {
			wp_enqueue_script( 'jquery',
				'https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js' ); // @TODO - Load locally
				...
				case 'advgb/button':
					...
					if ( defined( 'ADVANCED_GUTENBERG_PRO' ) && $this->settingIsEnabled( 'enable_advgb_blocks' ) ) {
						...
						if ( method_exists(
							...
							PPB_AdvancedGutenbergPro\Utils\Definitions::advgb_pro_enqueue_font_styles_frontend( 'advgb/button',
								$blockAttrs );
						}
					}
				...
				case 'advgb/login-form':
					...
					wp_localize_script(
						...
						[
							...
							'unregistrable_notice'  => __( 'User registration is currently not allowed.',
								'advanced-gutenberg' ),
