...
if ( ! class_exists( 'AdvancedGutenbergMain' ) ) {
    ...
    float: left;'
		...
		public function addEditorAssetsSiteEditor() {
			...
			if ( Utilities::settingIsEnabled( 'enable_advgb_blocks' ) && $pagenow === 'site-editor.php' ) {
		...
		public function enqueueEditorAssets() {
			...
			if (
				Utilities::settingIsEnabled( 'enable_custom_styles' )
					...
					function ( $settings ) {
						$settings['advanced-gutenberg'] = [
							...
							'screens' => [
								...
								[ 'base' => 'blocks_page_advgb_block_usage' ],
		...
		public function scan_block_usage() {
			...
			foreach ($query->posts as $post) {
				...
				$post_data = [
					'post_id' => $post_id,
					'post_title' => $post->post_title ?: "Post ID: $post_id",
				...
				foreach ($blocks as $b) {
					...
					$post_data['blocks'][] = [
						'name' => $name,
						'clientId' => $b['clientId'] ?? '',
						'innerBlocks' => array_map(function($inner) {
							return [
				...
				foreach ($block_counts as $name => $count) {
					if (!isset($usage[$name])) {
						$usage[$name] = [
							'posts' => [],
							'total' => 0
					...
					$usage[$name]['posts'][$post_id] = [
						'count' => $count,
						'scanned' => $current_time
					];
					$usage[$name]['total'] += $count;
				}
		...
		public function loadBlockUsagePage() {
			...
			// Block usage is disabled
			if ( ! Utilities::settingIsEnabled( 'enable_block_usage' ) ) {
				wp_die(
					sprintf(
						esc_html__(
							'This feature is disabled. In order to use, please enable back through %sDashboard%s.',
		...
		public function blocksUsageData( ) {
		 ...
		 * Block Usage page
		 */
		public function blocksUsagePage() {
			?>
            <div class="publishpress-admin wrap">
                <header>
                    <h1 class="wp-heading-inline">
		...
		public function setFrontendAssets( $content ) {
			...
			if ( isset( $_GET['customize_theme'] ) ) {
				...
				// Pro
				if ( defined( 'ADVANCED_GUTENBERG_PRO_LOADED' ) && Utilities::settingIsEnabled( 'enable_advgb_blocks' ) ) {
					if ( method_exists( 'PPB_AdvancedGutenbergPro\Utils\Definitions',
		 ...
		 * @deprecated 3.2.0 Use Utilities::settingIsEnabled() instead
         *
		 * @return boolean
		 */
		public function settingIsEnabled( $setting ) {
			return Utilities::settingIsEnabled( $setting );
		}
		...
		public function advgb_SetStylesForBlocks( $blockAttrs, $blockName ) {
			...
			// Load common CSS
			if ( in_array( $blockName, $availableBlocks ) && Utilities::settingIsEnabled( 'enable_advgb_blocks' ) ) {
				wp_enqueue_style(
					'advgb_blocks_styles',
					ADVANCED_GUTENBERG_PLUGIN_DIR_URL . 'assets/css/blocks.css',
					array(),
